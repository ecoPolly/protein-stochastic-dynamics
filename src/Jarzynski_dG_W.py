...
class MyApp(QtWidgets.QMainWindow, Ui_MainWindow):
    ...
    def plot_average_trajectory(self):
        ...
        self.grafico.legend(fontsize=6.5)
    ...
    def compute_w_protein(self, show_plot=False):
        ...
        if show_plot:
            ...
            ax.set_xlabel("Extension (nm)", fontsize=10)
    ...
    def compute_fjc_work(self, show_plot=True):
        ...
        if show_plot:
            ...
            plt.clf() 
            plt.plot(fjc_x, F, color='white', lw=1.5, label="FJC Model")
            plt.fill_betweenx(F_orient, x_orient, x_orient[-1], 
                             color="green", alpha=0.4, label=f"W_orient: {W_orient:.2f}")
            plt.fill_betweenx(F_stretch, x_stretch, x_stretch[-1], 
                             color="red", alpha=0.4, label=f"W_stretch: {W_stretch:.2f}")

            plt.axhline(y=f0, color='yellow', linestyle='--', alpha=0.6, label=f"F_min ({f0} pN)")
            plt.axhline(y=fmax, color='orange', linestyle='--', alpha=0.6, label=f"F_max ({fmax} pN)")
